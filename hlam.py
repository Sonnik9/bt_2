...
#         print(f"{key}: {value}")
    ...
    # mainn_init.dfpl.reset_index(inplace=True)

    # # Проверка формата столбца 'Date' и его преобразование в datetime
    # mainn_init.dfpl['Date'] = pd.to_datetime(mainn_init.dfpl['Date'])

    # # Затем устанавливаем индекс
    # mainn_init.dfpl.set_index('Date', inplace=True)
...
# # bt.plot(show_legend=False)
    ...
    #     for row in range(SLbackcandles, len(data)):
    #         if SLbackcandles == 0:
    #             SLSignal[row] = data['High'].iloc[row] if data['ordersignal'].iloc[row] == 1 else data['Low'].iloc[row]
    #         else:
    #             mi = 1e10
    #             ma = -1e10
